...
allLanguages.sort()
let setOfLanguages = new Set(allLanguages)
...
function mostSpokenLanguages (arr, count) {
    let resultObj = []
    for (const law of setOfLanguages) {
        let result = arr.filter(ele => ele === law)
        let obj = {
            language: law,
            spoken: result.length
        }
        resultObj.push(obj)
    }
    resultObj.sort((a,b) => {
        return b.spoken - a.spoken
    })
    return resultObj.slice(0, count)
}
console.log(mostSpokenLanguages(allLanguages, 10))
